		...
		<div className='inline-flex items-center z-10'>
			...
			<div className={`flex ml-4 space-x-4 ${navState ? 'hidden' : ''}`}>
				<button
					...
					className='text-3xl group dark:hover:text-[#0ff] hover:text-[#000080] hover:scale-125 lg:hover:animate-pulse duration-75 hover:shadow-none dark:hover:shadow-none'
				>
					{props.darkMode ? (
						<BsSun className='group-hover:hidden' />
					) : (
						<BsMoon className='group-hover:hidden' />
					)}
					{props.darkMode ? (
						<BsFillSunFill className='hidden group-hover:block' />
					) : (
						<BsFillMoonFill className='hidden group-hover:block' />
					)}
				</button>
				<a href={Resume} download='OdnelTrumboreSiteResume.pdf' target='_blank'>
					<div>
						<Buttons style={null} onClick={null}>
							...
							<svg
								className='fill-current w-4 h-4 ml-2 animate-bounce duration-100'
			...
			<button
				...
				className='flex text-3xl z-10 lg:hidden ml-4'
			>
			...
			<div className={navState ? ' ease-linear duration-300' : 'hidden'}>
				<ul className='absolute text-2xl top-0 right-0 w-[50%] h-screen dark:bg-[#083248] bg-[#cacac2] dark:bg-opacity-90 bg-opacity-90 flex flex-col justify-start items-center space-y-6 pt-24 rounded-l-xl'>
					...
					<li className=''>
						...
						>
							{' '}
							{/*change offset after adding in more projects */}
							Projects
						</Link>
					</li>
					<li className=''>
